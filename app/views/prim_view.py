...
from algorithms.prim import executer_prim
...
class PrimView(tk.Toplevel):
    def __init__(self, parent, reseau):
        super().__init__(parent)
        self.reseau  = reseau
        self._parent = parent

        self.title("Algorithme de Prim — Arbre Couvrant Minimal")
    ...
    def _creer_interface(self):
        creer_header(self, "  Algorithme de Prim — Arbre Couvrant Minimal")
        ...
        tk.Label(
            outer,
            text=("Construit l'Arbre Couvrant Minimal (ACM) par croissance progressive "
                  "à partir d'un nœud source, en sélectionnant l'arête de coût minimal."),
    ...
    def _calculer(self):
        ...
        res = executer_prim(self.reseau.graphe)
    ...
    def _afficher(self, res):
        ...
        tk.Label(card_top, text="Résultat Prim — ACM",
        ...
        for col, (lbl, val) in enumerate([
            ("Arêtes sélectionnées", str(len(liaisons))),
            ...
            tk.Label(c, text=val,  font=(POLICE, 14, 'bold'),  bg='#0F172A', fg=SUCCES).pack()

        # Tableau des liaisons
        card_table = tk.Frame(self._frame_resultat, bg=CARTE, padx=18, pady=14)
        card_table.pack(fill=tk.BOTH, expand=True)

        tk.Label(card_table, text="Liaisons sélectionnées",
    ...
    def _voir_graphe(self, acm):
        ...
        GraphView(self._parent, self.reseau, acm=acm,
                  titre="Prim — Arbre Couvrant Minimal")

    def _voir_graphe_acm(self):
        """Affiche le graphe complet avant tout calcul"""
        from .graph_view import GraphView
        GraphView(self._parent, self.reseau, titre="Graphe complet — Prim")
